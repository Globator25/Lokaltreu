...
2. Sammle Logs/Metrics (401/403/5xx) und relevante `correlation_id` für Evidenz.
...
- **Verifikation:** JWKS-Lint/Healthcheck erfolgreich; Stage/Prod liefern erwarteten `kid`; Logs zeigen erfolgreiche Verifikation.
...
1. Führe synthetische Checks (Login, Refresh, Device-Proof Dry-Run) sowie Smoke-Tests durch.
...
- UTC-Timeline (Freeze, Deploy, Validation), Hash/Fingerprint der wiederhergestellten JWKS-Version, Begründung für Key-Entzug.
- Logs/Metrics-Exports (`jwt_validation_errors`, 401/403/5xx), Smoke-Test-Protokolle, `correlation_id`-Referenzen (PII-frei).
